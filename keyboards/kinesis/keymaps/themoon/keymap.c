...
void f5_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  if (!is_after_lead(KC_F5, true)) {
    switch (f5_tap_state.state) {
      ...
      case DOUBLE_TAP:
        if (isMac) { down(KC_LGUI); key_code(KC_F5); break; }
        else if (isWin) { down(KC_LCTL); key_code(KC_F5); break; }
      case DOUBLE_HOLD:
         down(KC_LALT); key_code(KC_F5); break;
...
void dynamic_macro_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  if (!is_after_lead(isMac ? KC_NUBS : KC_PAUS, true)) {
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case _1_BSLS: { return lead_custom_autoshifted(KC_1, KC_BSLS, KC_NO, pressed); }
        ...
        case KC_RGUI: { if (is_after_lead(KC_DEL, pressed)) { return false; } return true; }
        ...
        case ALT_EQL_MAC: {
          ...
          momentary_layer_tap(KC_EQL, KC_NO, KC_LALT, KC_NO, KC_NO, KC_NO, &alt_eql_mac_layer_timer, &alt_eql_mac_interrupted, pressed, 250, true);
          return true;
        }
        ...
        case ALT_EQL_WIN: {
          ...
          momentary_layer_tap(KC_EQL, KC_NO, KC_LALT, KC_NO, KC_NO, KC_NO, &alt_eql_win_layer_timer, &alt_eql_win_interrupted, pressed, 250, false);
          return true;
        }
